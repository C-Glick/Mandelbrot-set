...
/**
 * The same as the complex class but uses the BigDecimal class to create high
...
public class BigComplex {
	BigDecimal real;
	BigDecimal imag;
	static MathContext rMode = new MathContext(32, RoundingMode.HALF_UP); // Rounding mode to round towards "nearest
																			// neighbor" unless both neighbors are
																			// equidistant, in which case round up.

	/**
	 * Complex number example: (2+5i), real = 2, imag = 5. Constructor for a
	 * BigComplex object. Uses BigDecimal objects to create high Precision objects.
	 * when creating new BigDecimal objects, use strings rather than doubles for the
	 * best results, 'new BigDecimal("48.02761");' not 'new BigDecimal(someDouble);'
	 * 
	 * @param real A BigDecimal object to represent the real part of the complex
	 *             number
	 * @param imag A BigDecimal object to represent the imaginary part of the
	 *             complex number
	 */
	...
	/**
	 * Create a new infinite precision complex number from a low precision complex
	 * number
	 * 
	 * @param complex The low precision complex number to convert from
	 */
	public BigComplex(Complex complex) {
		this.real = new BigDecimal(complex.getReal());
		this.imag = new BigDecimal(complex.getImag());
	}

	/**
	 * Create a new infinite precision complex number from a high precision complex
	 * number
	 * 
	 * @param complexHP The high precision complex number to convert from
	 */
	public BigComplex(DoubleDoubleComplex complexHP) {
		this.real = new BigDecimal(complexHP.getReal().toStandardNotation());
		this.imag = new BigDecimal(complexHP.getImag().toStandardNotation());
	}
	...
	public void multiply(BigComplex num) {
		...
		BigDecimal L = getImag().multiply(num.getImag(), rMode); // will give some number*i^2, i^2 is the same as -1 so
																	// this number should be inverted
	...
	public void setCalcPrecision(int value) {
		rMode = new MathContext(value, RoundingMode.HALF_UP); // Rounding mode to round towards "nearest neighbor"
																// unless both neighbors are equidistant, in which case
																// round up.
																// Updates the rMode object (rounding mode).
																// rMode is static so it changes for all BigComplex
																// objects
	}

	/**
	 * Returns the maximum number of digits used for all BigComplex calculations.
	 * 
	 * @return The current rMode value defined in the BigComplex Class
	 */
	public static MathContext getRMode() {
		return rMode;
	}
}

...
/**
 * The same as the complex class but uses the DoubleDouble class to create high
...
public class DoubleDoubleComplex {
	 ...
	 * DoubleDoubleComplex object. Uses DoubleDouble objects to create high
	 * Precision objects. when creating new DoubleDouble objects, use strings rather
	 * than doubles for the best results, 'new DoubleDouble("48.02761");' not 'new
	 * DoubleDouble(someDouble);'
	 * 
	 * @param real A DoubleDouble object to represent the real part of the complex
	 *             number
	 * @param imag A DoubleDouble object to represent the imaginary part of the
	 *             complex number
	 */
	public DoubleDoubleComplex(DoubleDouble real, DoubleDouble imag) {
		this.real = real;
		this.imag = imag;
	}

	/**
	 * Create a new high precision complex number from a low precision complex
	 * number
	 * 
	 * @param complex The low precision complex number to convert from
	 */
	public DoubleDoubleComplex(Complex complex) {
		this.real = new DoubleDouble(complex.getReal());
		this.imag = new DoubleDouble(complex.getImag());
	}

	/**
	 * Create a new high precision complex number from an infinite precision complex
	 * number
	 * 
	 * @param complexIP The infinite precision complex number to convert from
	 */
	public DoubleDoubleComplex(BigComplex complexIP) {
		this.real = new DoubleDouble(complexIP.getReal().toPlainString());
		this.imag = new DoubleDouble(complexIP.getImag().toPlainString());
	}
	...
	public void multiply(DoubleDoubleComplex num) {
		...
		DoubleDouble L = getImag().multiply(num.getImag()); // will give some number*i^2, i^2 is the same as -1 so this

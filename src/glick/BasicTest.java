...
/**
 * Test for just one complex number, sets the resultsArray in the Launcher class
 * to the number of iterations it took to reach the threshold. Will be 0 if
 * iteration limit is reached, and thus, is in the Mandelbrot set. *
 */
public class BasicTest extends RecursiveAction {
	private static final long serialVersionUID = -3661407454480473902L;
	 ...
	 * @param threshold Threshold for the test, stops once the value grows higher
	...
	protected void compute() {
		...
		for (int i = 1; i <= limit; i++) {
			...
			double value = z.abs();
			if (value >= threshold) {
				if (smoothingCount >= 2) { // iterate 3 more times to reduce the error when calculating the smoothing
											// algorithm
					result = smoothShading(i, value); // figure out what the required number of iterations should be (ex
														// 3.52233 vs 3)
					break;
				}
				smoothingCount++;

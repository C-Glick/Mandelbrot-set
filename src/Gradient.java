...
/**
 * A class made up of a list of GradientPoint objects.
 * A linear gradient is generated between each defined point.
 * It is recommend to always define a point at position 0.0 and 1.0
 *
 */
public class Gradient {
	 ...
	 */
	Gradient(GradientPoint point1, GradientPoint point2){
		listOfPoints=new ArrayList<GradientPoint>();
	...
	/**
	 * Add a point to the gradient. It is not necessary to add
	 * points in the order in which they appear in the gradient.
	 * The list of gradient points will be sorted when a new point is added.
	 * @param point
	 */
	...
	public Color getColor(double position) {
		...
		for(int i=0; i<listOfPoints.size(); i++) {
			...
			if((currentPoint.getPosition()<=position && listOfPoints.get(i+1).getPosition()>=position)) {
				 ...
				 //calculate the value of the RGB channels at the local position between the two points
